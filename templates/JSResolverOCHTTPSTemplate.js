            ...
            if (field.arguments.length > 0) {
                field.arguments.forEach(arg => {
                    if (arg.type.kind === 'NamedType') {
                        getSchemaInputTypeArgs(arg.type.name.value, r.agrs);
                    } else if (arg.type.kind === 'NonNullType') {
                        getSchemaInputTypeArgs(arg.type.type.name.value, r.agrs);
                    } else if (arg.type.type.name.value === 'String' || arg.type.type.name.value === 'Int' || arg.type.type.name.value === 'ID') {
                        r.agrs.push({name: arg.name.value, type: arg.type.type.name.value});
                    } 
    ...
    if (querySchemaInfo.graphQuery != null) {
        ...
        obj.definitions[0].selectionSet.selections[0].arguments.forEach(arg => {
            if (arg.name.value == 'filter') {
                ...
                queryArguments = queryArguments + inputFields + ",";                
            } else if (arg.name.value == 'id') {
                where =` WHERE ID(${querySchemaInfo.pathName}) = '${arg.value.value}'`;
            } else {
...
function createQueryFieldMatchStatement(withId, fieldSchemaInfo, lastNamePath) {
...
function createQueryFieldLeafStatement(withId, fieldSchemaInfo, lastNamePath) {      
    ...
    if (fieldSchemaInfo.alias === 'id' && fieldSchemaInfo.graphQuery == null) {          
        ...
        if (fieldSchemaInfo.graphQuery !=null ) {
            if (useCallSubquery) {
                ...
                withId++;
                createQueryFieldMatchStatement(withId, fieldSchemaInfo, lastNamePath);
            }
        } else {              
...
function createTypeFieldStatementAndRecurse(e, withId, fieldSchemaInfo, lastNamePath, lastType) {
    ...
    if (schemaTypeInfo.isRelationship) {        
        if (schemaTypeInfo.relationship.direction === 'IN') {
            matchStatements.push(`OPTIONAL MATCH (${lastNamePath})<-[${schemaTypeInfo.pathName}_${schemaTypeInfo.relationship.edgeType}:${schemaTypeInfo.relationship.edgeType}]-(${schemaTypeInfo.pathName}:${schemaTypeInfo.typeAlias})`);
        } else {
            matchStatements.push(`OPTIONAL MATCH (${lastNamePath})-[${schemaTypeInfo.pathName}_${schemaTypeInfo.relationship.edgeType}:${schemaTypeInfo.relationship.edgeType}]->(${schemaTypeInfo.pathName}:${schemaTypeInfo.typeAlias})`);
        }
    } 
    ...
    selectionsRecurse(e.selectionSet.selections, thisWithId, schemaTypeInfo.pathName, schemaTypeInfo.type);        
    withStatements[thisWithId].content += '}';
...
function selectionsRecurse(s, withId, lastNamePath, lastType) {
    ...
    s.forEach(e => {
        ...
        if (!fieldSchemaInfo.isSchemaType) {             
            createQueryFieldLeafStatement(withId, fieldSchemaInfo, lastNamePath);            
            // exit terminating recursion branch
            return
        }

        withId ++;
        createTypeFieldStatementAndRecurse(e, withId, fieldSchemaInfo, lastNamePath, lastType)                          
    });    
};
...
function resolveGrapgDBqueryForGraphQLQuery (obj, querySchemaInfo) {
    ...
    selectionsRecurse(obj.definitions[0].selectionSet.selections[0].selectionSet.selections, 0, querySchemaInfo.pathName, querySchemaInfo.returnType);
...
function transformFunctionInputParameters(fields, querySchemaInfo) {
    let r = '';
    querySchemaInfo.agrs.forEach(arg => {
...
function resolveGrapgDBqueryForGraphQLMutation (obj, querySchemaInfo) {
    ...
    */
    ...
    if (querySchemaInfo.name.startsWith('createNode') && querySchemaInfo.graphQuery == null) {
        ...
        let ocQuery = `CREATE (${nodeName}:${querySchemaInfo.returnTypeAlias} {${inputFields}})\nRETURN ${returnBlock}`;        
        return ocQuery;
    }
    ...
    if (querySchemaInfo.name.startsWith('updateNode') && querySchemaInfo.graphQuery == null) {
        ...
        let inputFields = transformFunctionInputParameters(obj.definitions[0].selectionSet.selections[0].arguments[1].value.fields, querySchemaInfo);
        let nodeName = querySchemaInfo.name + '_' + querySchemaInfo.returnType;
        let returnBlock = `ID(${nodeName})`;
        if (obj.definitions[0].selectionSet.selections[0].selectionSet != undefined) {        
            returnBlock = returnStringOnly(obj.definitions[0].selectionSet.selections[0].selectionSet.selections, querySchemaInfo);
        ...
        // workaround:
        let propertyList = inputFields.split(', ');
        let setString = '';
    ...
    if (querySchemaInfo.name.startsWith('connectNode') && querySchemaInfo.graphQuery == null) {
        ...
        if (obj.definitions[0].selectionSet.selections[0].arguments.length > 2) {            
            ...
            let ocQuery = `MATCH (from), (to)\nWHERE ID(from) = '${fromID}' AND ID(to) = '${toID}'\nCREATE (from)-[${edgeName}:${egdgeTypeAlias}{${inputFields}}]->(to)\nRETURN ${returnBlock}`;
            return ocQuery;
        } else {
    ...
    if (querySchemaInfo.graphQuery != null) {
        ...
        if (ocQuery.includes('$input')) {
            let inputFields = transformFunctionInputParameters(obj.definitions[0].selectionSet.selections[0].arguments[0].value.fields, querySchemaInfo);
            ocQuery = ocQuery.replace('$input', inputFields);
        } else {
            obj.definitions[0].selectionSet.selections[0].arguments.forEach(arg => {
