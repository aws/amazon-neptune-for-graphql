...
Input a changes file
 ...
 - Changes file format: [
     { "action": "add",    "type": "graphQLTypeName", "field": "graphQLFieldName", "value": "fieldDefinition" },
     { "action": "remove", "type": "graphQLTypeName", "field": "graphQLFieldName" },
     { "action": "addType", "value": "type NewType { field: String }" }
   ]
   The "addType" action appends a new type definition to the schema and takes no "type" or "field" properties.
   The "addType" action is appended after add/remove processing, so add actions against a type declared in the same file will not find their target.
...
Create, update or remove the AWS resources pipeline for the AppSync GraphQL API
 ...
 - it creates or update the AWS Lambda, in the Neptune DB VPC, set the environment variables, 
   and upload the resolver code generated by the utility.
...
Create Apollo Server Artifacts
- generate a ZIP file of Apollo Server or Subgraph artifacts instead of App Sync
